...
#     else:
#         loss += NewDecorrelationBetweenDim1(output, 1)
#     return loss
def blockDecorelation(output):
    # flag = 0
    loss = 0
...
def top5_accuracy(output, label):
    _, predicted = output.topk(5, 1, True, True)  # 获取概率最高的前5个预测结果
    predicted = predicted.t()  # 转置
    correct = predicted.eq(label.view(1, -1).expand_as(predicted))  # 判断预测结果是否与实际标签一致
    top5_acc = correct[:5].view(-1).float().sum(0, keepdim=True)  # 统计前5个正确的个数
    return top5_acc.item() / label.size(0)  # 返回top5准确率
...
def NewDecorrelationBetweenDim1(input2, scale):
    input2 = input2.view(input2.size(0), input2.size(1), -1) #通过view函数将最后两个维度合并为一个
    input2 = input2.permute(1, 0, 2)#交换了input2的维度顺序
    input_avg = torch.mean(input2, dim=1, keepdim=True)#求第一个维度的平均值
    input_subAvg = input2 - input_avg#第一个维度
    input_subAvg1 = input_subAvg.reshape(input_subAvg.size(0), -1)
    newTensor = torch.mm(input_subAvg1, input_subAvg1.T) / input_subAvg.size(2)
    input_sum = torch.sum(input_subAvg1.pow(2), dim=1, keepdim=True)
    input_sum = pow(input_sum / input_avg.size(2), 0.5)
    ...
    return scale * res_loss
# input2 = input2.view(input2.size(0), input2.size(1), -1)：该行代码将输入的张量input2的形状改变为(8, 8, 1024)。它通过view函数将最后两个维度合并为一个，并根据原始张量的大小和其他维度的大小自动确定新的维度的大小。
#
# input2 = input2.permute(1, 0, 2)：该行代码交换了input2的维度顺序，使其变为(8, 8, 1024)。通过permute函数，第一维和第二维被交换。
#
# input_avg = torch.mean(input2, dim=1, keepdim=True)：该行代码计算了在第一维度上的平均值，得到维度为(8, 1, 1024)的张量input_avg。dim=1表示沿着第一维度计算平均值，keepdim=True表示保持结果的维度。
#
# input_subAvg = input2 - input_avg：该行代码计算了input2与input_avg之间的差异，得到维度为(8, 8, 1024)的张量input_subAvg。这样做可以将每个元素减去对应维度的平均值。
#
# input_subAvg1 = input_subAvg.reshape(input_subAvg.size(0), -1)：该行代码将input_subAvg的形状改变为(8, 8192)，通过reshape函数将最后两个维度合并。结果是一个二维张量。
#
# newTensor = torch.mm(input_subAvg1, input_subAvg1.T) / input_subAvg.size(2)：该行代码计算了input_subAvg1与其转置矩阵相乘，并除以input_subAvg的第三个维度大小。结果是一个形状为(8, 8)的张量。
#
# input_sum = torch.sum(input_subAvg1.pow(2), dim=1, keepdim=True)：该行代码计算了input_subAvg1在第二个维度上的平方和，得到一个形状为(8, 1)的张量input_sum。pow(2)表示进行平方运算。
#
# input_sum = pow(input_sum / input_avg.size(2), 0.5)：该行代码将input_sum除以input_avg的第三个维度大小，然后进行0.5次乘方运算。结果是一个形状为(8, 1)的张量。
#
# ret_sum = torch.mm(input_sum, input_sum.T).clamp(min=1e-12)：该行代码计算了input_sum与其转置矩阵相乘，并对结果进行截断，确保所有元素的最小值不低于1e-12。
#
# res = newTensor / ret_sum：该行代码将newTensor除以ret_sum得到结果，得到一个形状为(8, 8)的张量res。
#
# res_loss = torch.sum(pow(res, 2)) - torch.sum(pow(res.diagonal(), 2))：该行代码计算了res每个元素的平方和减去res对角线上元素的平方和。结果是一个标量。
#
# res_loss = res_loss / max((res.size(0) - 1) * res.size(0), 1e-12)：该行代码将res_loss除以(res.size(0) - 1) * res.size(0)，确保分母不为零。如果分母为零，则使用1e-12代替。
...
def train_soft_0602(net, train_data, valid_data, cfg, criterion, criterion1, criterion2, save_folder, save_txt, classes_num):
    ...
    for epoch in range(cfg['max_epoch']):
        ...
        # with torch.no_grad():
        if (epoch + 1) == 150:
            torch.save(output, save_folder + save_txt + str(epoch+1) + '.pth')
        #     # if (epoch + 1) > 83:
        #     torch.save(output, save_folder + save_txt + str(epoch + 1) + 'epoch_output.pth')
        cur_time = datetime.now()
        h, remainder = divmod((cur_time - prev_time).seconds, 3600)
        m, s = divmod(remainder, 60)
        time_str = "Time %02d:%02d:%02d" % (h, m, s)
